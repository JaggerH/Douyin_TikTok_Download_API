...
_COOKIE_MAX_AGE_SECONDS = 24 * 3600  # refresh if older than 24 h


async def _startup_refresh_youtube_cookies():
    """Pull YouTube cookies from CookieCloud on startup if stale or missing."""
    from crawlers.utils.cookie_refresher import get_refresher
    import logging

    _log = logging.getLogger("video-service.startup")

    # Check if cookie file is fresh (< 24 h old)
    if os.path.exists(_COOKIE_FILE):
        age = time.time() - os.path.getmtime(_COOKIE_FILE)
        if age < _COOKIE_MAX_AGE_SECONDS:
            _log.info(f"YouTube cookies still fresh ({int(age/3600)}h old), skip CookieCloud pull")
            return

    refresher = get_refresher()
    if not refresher.available:
        _log.warning("CookieCloud not configured — skipping startup cookie refresh")
        return

    _log.info("Pulling YouTube cookies from CookieCloud on startup...")
    try:
        ...
        netscape_lines = ["# Netscape HTTP Cookie File", "# Generated by video-service startup", ""]
        ...
        _log.info(f"YouTube cookies written on startup ({len(yt_cookies)} youtube + {len(google_cookies)} google = {total} total) → {_COOKIE_FILE}")
    except Exception as exc:
        _log.error(f"Startup YouTube cookie refresh failed: {exc}")


@asynccontextmanager
async def lifespan(application: FastAPI):
    await _startup_refresh_youtube_cookies()
    yield
